...
# Removing a user in the contacts table in MySql
    if value == "d":   
...
# Outputting all contacts by creation date
    if value =="c":
        select_creationDate = "SELECT * FROM contacts ORDER BY creationDate" #used ORDER BY to sort the names in my creationDate column
        creationDate = execute_read_query(connection, select_creationDate)
        for DateOfCreation in creationDate:
            print(DateOfCreation[2])
        continue
